...
function startApi() {
  return new Promise((resolve, reject) => {
    exec("python launch.py --api", { cwd: "path_to_stable_diffusion_webui_folder" }, (error, stdout, stderr) => {
      if (error) {const { PREFIX } = require("../../krampus");
const { WarningError } = require("../../errors/WarningError");
const fs = require("fs");
const axios = require("axios");
const { exec } = require("child_process");
...
function startApi() {
  return new Promise((resolve, reject) => {
    // Especifica la ruta completa a python.exe si es necesario
    exec("C:\\Python39\\python.exe launch.py --api", { cwd: "path_to_stable_diffusion_webui_folder" }, (error, stdout, stderr) => {
      if (error) {
